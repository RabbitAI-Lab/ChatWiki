...
export async function PUT(req: NextRequest) {
  ...
  if (body.mcpJson !== undefined) {
    ...
    if (_op?.serverName && (_op.action === "enable" || _op.action === "disable")) {
      ...
      console.log(`[MCP] ${label} server "${_op.serverName}" in workspace [${dirSegments.join("/")}]`);
    } else {
      logOperation({
        projectId: extractProjectId(dirSegments),
        category: "mcp",
    ...
    if (Object.keys(normalized._apiKeys || {}).length > 0) {
      console.log(`[MCP] .mcp-config.json after write:\n${JSON.stringify({ _apiKeys: normalized._apiKeys }, null, 2)}`);
    }
