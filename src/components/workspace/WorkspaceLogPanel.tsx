...
interface WorkspaceLogPanelProps {
...
export default function WorkspaceLogPanel({
  workspacePath,
  ...
  const fetchLogs = useCallback(
    async (pageNum: number, category: LogCategory | "all") => {
      setLoading(true);
      try {
  ...
  useEffect(() => {
    ...
    Promise.resolve()
      ...
      .then(() => authFetch(`/api/fs/workspace-logs?${params}`))
  ...
  }, [selectedCategory, workspaceId, authFetch]);
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2">
                    ...
                    {actionLabel && (
                      <span
                        className={`px-1.5 py-0.5 text-[10px] font-medium rounded ${actionLabel.className}`}
                      >
