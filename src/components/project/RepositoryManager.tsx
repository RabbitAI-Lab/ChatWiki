...
export default function RepositoryManager({
  ...
  const [formCredType, setFormCredType] = useState<"none" | "token" | "username_password">("none");
        ...
        const updatedRepos = repositories.map((r) =>
          r.id === repo.id
            ? {
                ...
                lastSyncAt: data.success ? new Date().toISOString() : r.lastSyncAt,
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2">
                  ...
                  <span className={`inline-flex px-1.5 py-0.5 rounded text-[10px] font-medium ${TYPE_COLORS[repo.type] || TYPE_COLORS.other}`}>
                    {TYPE_LABELS[repo.type] || repo.type}
                  </span>
                  <RepositorySyncStatus repository={repo} />
                </div>
                <p className="text-xs text-gray-400 truncate">{repo.url}</p>
                {repo.errorMessage && (
                  <p className="text-xs text-red-500 truncate">{repo.errorMessage}</p>
                )}
              </div>
              {/* 同步按钮 */}
              <button
              ...
              >
                ...
                <span>{repo.syncStatus === "not_cloned" ? t('repository.clone') : t('repository.sync')}</span>
              </button>
              {/* 删除按钮 */}
              <button
